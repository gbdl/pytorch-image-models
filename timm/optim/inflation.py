...
class BBI(Optimizer): 
    ...
    """
    def __init__(self, params, lr=required, eps1=1e-10, eps2 = 1e-40, v0=0, threshold0 = 10, threshold = 150, deltaEn = 0.0, consEn = True, n_fixed_bounces = 5):
...
#This is with no bouncings at all
class BI(Optimizer): 
    """Optimizer based on the BBI model of inflation.
    Args:
        params (iterable): iterable of parameters to optimize or dicts defining
        ...
        consEn (bool): if True enforces energy conservation at every step
         """
    def __init__(self, params, lr=required, eps1=1e-10, eps2 = 1e-40, v0=0, deltaEn = 0.0, consEn = True):
            
            defaults = dict(lr=lr, eps1=eps1, eps2=eps2, v0=v0, deltaEn = deltaEn, consEn = consEn)
            self.energy = None
            self.iteration = 0
            self.deltaEn = deltaEn
            self.consEn = consEn 
            super(BI, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(BI, self).__setstate__(state)
